    ...
    public class CastController : Controller
    {
        ...
        public IActionResult sortList(int id)
        {
            ...
            (p => new CCastViewModel
            {
                ...
                英文名字nameEng = p.演員編號actor.演員英文名字nameEng,
        ...
        public IActionResult sortListDir(int id)
        {
            ...
            {
                director = q,
                中文名字nameCht = q.導演編號director.導演中文名字nameCht,
                英文名字nameEng = q.導演編號director.導演英文名字nameEng,
        ...
        public ActionResult Create(CCastViewModel p)
        {
            ...
            m = p.cast;
            m.演員編號actorId = db.演員總表actors.FirstOrDefault(q => q.演員中文名字nameCht == p.中文名字nameCht).演員編號actorsId;
        ...
        public ActionResult CreateDir(int? id)
        {
            if (id != null)
            {
                ...
                {
                    director = p,
                    中文名字nameCht = p.導演編號director.導演中文名字nameCht,
                    中文標題titleCht = p.電影編號movie.中文標題titleCht,
                }).FirstOrDefault();
        ...
        public ActionResult CreateDir(CMovieDirectorViewModel p)
        {
            ...
            m = p.director;
            m.導演編號directorId = db.導演總表directors.FirstOrDefault(q => q.導演中文名字nameCht == p.中文名字nameCht).導演編號directorId;
            //m.電影分級編號ratingId = db.電影分級movieRatings.FirstOrDefault(q => q.分級級數ratingLevel == p.分級級數ratingLevel).分級編號ratingId;
